...
static void emit_expr(Node *expr, struct definition_info *func, FILE *fp) {
  ...
  case NODE_UNARY:
    ...
    if (expr->text && expr->text[0] == '-')
      fprintf(fp, "    neg     rax\n");
    /* unary '+' is a no-op: value already in rax */
    break;
  ...
  case NODE_DOT: {
    ...
    struct variable_info *var =
        find_variable(var_node->text, func, globalTable, size);
    if (var && var->type_def) {
 ...
 * ========================================================= */
static void emit_instr_list(Node *instr, struct definition_info *func,
                            FILE *fp);
...
static void emit_instr(Node *node, struct definition_info *func, FILE *fp) {
  ...
  case NODE_WHILE: {
    ...
    if (body)
      emit_instr_list(body->firstChild, func, fp);
...
static void emit_function(Node *funcNode, FILE *fp) {
  ...
  struct definition_info *func_def =
      find_in_table_symbole(fname, globalTable, size);
  int stack_size = func_def ? func_def->total_size : 0;
  /* Ensure minimum 16 bytes and alignment */
  if (stack_size < 16)
    stack_size = 16;
  if (stack_size % 16 != 0)
  ...
  /* Prologue: identical for ALL functions including main */
  fprintf(fp, "    push    rbp\n");
  fprintf(fp, "    mov     rbp, rsp\n");
  fprintf(fp, "    sub     rsp, %d\n", stack_size);

  /* Save register-passed parameters into the stack frame.
   * AMD64 convention: rdi, rsi, rdx, rcx, r8, r9 for args 1-6.
   * We store them at their declared offsets in the activation record. */
  if (func_def) {
    const char *param_regs[] = {"rdi", "rsi", "rdx", "rcx", "r8", "r9"};
    const char *param_regs8[] = {"dil", "sil", "dl", "cl", "r8b", "r9b"};
    const char *param_regs32[] = {"edi", "esi", "edx", "ecx", "r8d", "r9d"};
    int pidx = 0;
    struct variable_info *p = func_def->liste_champs;
    while (p && pidx < 6) {
      if (p->type_nature == CHAR)
        fprintf(fp, "    mov     byte [rbp - %ld], %s\n", (long)p->adresse + 8,
                param_regs8[pidx]);
      else if (p->type_nature == STRUCTURE && p->type_def)
        /* For struct params, store the full 64-bit value (pointer-like) */
        fprintf(fp, "    mov     qword [rbp - %ld], %s\n", (long)p->adresse + 8,
                param_regs[pidx]);
      else
        fprintf(fp, "    mov     dword [rbp - %ld], %s\n", (long)p->adresse + 8,
                param_regs32[pidx]);
      pidx++;
      p = p->next;
    }
  ...
  if (suite)
    emit_instr_list(suite->firstChild, func_def, fp);
...
void parcours_instruction(Node *root, struct table_symbole *gtable, FILE *fp) {
  ...
  /* _start entry point: call main, then exit with its return value */
  fprintf(fp, "\n_start:\n");
